...
switch($_GET[act]){
  ...
  default:
    ...
    echo "<h2>Exhibition</h2>
          <input type=button value='Add' onclick=location.href='?module=dtpameran&act=tambahpameran'>
          <table class='bordered'>
          <tr><th>no</th><th>exhibition</th><th>date</th><th>location</th><th>status</th><th>action</th></tr>"; 
    $tampil=mysql_query("SELECT * FROM tour_marketing WHERE DateTo >= '$today' ORDER BY DateFrom ASC ");
    $no=1;
    while ($r=mysql_fetch_array($tampil)){
            if($r[InhouseBSO]==''){$inbso="<a href=?module=dtpameran&act=crewpameran&id=$r[MarketingID]>Staff</a>";}else{{$inbso='Staff';}}
        ...
        echo "<tr><td>$no</td>
             ...
             <td>$r[Place]</td>
             <td>$r[Status]</td>  
             <td><center>$inbso | <a href=?module=dtpameran&act=editpameran&id=$r[MarketingID]>Edit</a> 
             </td></tr>";
      $no++;
    }
    echo "</table>";
  ...
  case "tambahpameran":
    ...
    echo "<h2>New Exhibition</h2>
          <form name=example method=POST onsubmit='return validateFormOnSubmit(this)' action='./aksi.php?module=dtpameran&act=input'>
          <table class='bordered'>
          <tr><td>Type</td> <td>   
          <input type=radio name='type' value='GENERAL' onclick='outhouse()' checked>&nbsp;General
          <input type=radio name='type' value='INHOUSE' onclick='inhouse()'>&nbsp;In House
          <input type=radio name='type' value='WOP' onclick='outhouse()'>&nbsp;World Of Panorama
          </td></tr> 
          <tr><td>Handle by</td>  <td>  <select name='inhousebso' disabled>
          <option value='' selected>- SELECT BSO -</option>";
          $tampils=mysql_query("SELECT * FROM tbl_msoffice where active='1' ORDER BY office_name");
            while($r1=mysql_fetch_array($tampils)){
                if($r[InhouseBSO]==$r1[office_code]){
    ...
    echo "</select></td></tr>
          ...
          <tr><td>Location</td><td><input type=text name='place' size='40'></td></tr>       
          ...
          <input type='checkbox' name='includepromo' value='YES' checked> Include Product Promo</td></tr></table>
          </td></tr>
  ...
  case "editpameran":
    $edit=mysql_query("SELECT * FROM tour_marketing WHERE MarketingID ='$_GET[id]'");
    ...
    echo "<h2>Edit Exhibition</h2>
          <form name='example' method=POST action=./aksi.php?module=dtpameran&act=update>
          <input type=hidden name=id value='$r[MarketingID]'>
          <table class='bordered'>
          <tr><td>Type</td> <td>   
          <input type=radio name='type' value='GENERAL' onclick='outhouse()'";if($r[Type]=='GENERAL'){echo"checked";}echo">&nbsp;General
          <input type=radio name='type' value='INHOUSE' onclick='inhouse()'";if($r[Type]=='INHOUSE'){echo"checked";}echo">&nbsp;In House
          <input type=radio name='type' value='WOP' onclick='outhouse()'";if($r[Type]=='WOP'){echo"checked";}echo">&nbsp;World Of Panorama
          </td></tr> 
          <tr><td>Handle by</td>  <td>  <select name='inhousebso' $inbso>
          <option value='' selected>- SELECT BSO -</option>";
          $tampils=mysql_query("SELECT * FROM tbl_msoffice where active='1' ORDER BY office_name");
            while($r1=mysql_fetch_array($tampils)){
                if($r[InhouseBSO]==$r1[office_code]){
    ...
    echo "</select></td></tr>
          <tr><td>Event Name</td><td><input type=text name='event' size='50' value='$r[Event]'></td></tr>
          ...
          - To <input type=text value='$DT' name='dateto' size='10'  onClick="."cal.select(document.forms['example'].dateto,'anchor2','dd-MM-yyyy'); return false;"." NAME='anchor2' ID='anchor2'>
           <font color='red'>(dd-mm-yyyy)</font></td></tr>
          <tr><td>Location</td><td><input type=text name='place' value='$r[Place]' size='40'></td></tr>      
          <tr><td>Extra Disc</td><td>
          <table style='border:0'><tr><td style='border:0'>
          ...
          <tr><td colspan='2' style='border:0'>
          <input type='checkbox' name='includepromo' value='YES' ";if($r[IncludePromo]=='YES'){echo"checked";}echo"> Include Product Promo</td></tr></table>
          </td></tr>
    ...
    echo "<h2>Staff for $r[Event]</h2>
          <form method='POST' name='example' action=./aksi.php?module=crewpameran&act=update>
          <input type=hidden name=id value='$r[MarketingID]'>
          <input type=hidden name='event' value='$r[event]'>
          <table id='crew' border='1' class='bordered'>";
           $i=0;
           $coba=mysql_query("SELECT * FROM tour_crewpameran where IDEvent ='$_GET[id]' ");
