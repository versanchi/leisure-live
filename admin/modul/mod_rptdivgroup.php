...
switch($_GET[act]){
  // Tampil Office
   default:
	...
	$Booking=mysql_query("SELECT TCDivision,Destination,tour_msproduct.SellingCurr,sum(if(GroupType='TMR' ,AdultPax+ChildPax,0)) as PaxTMR,sum(if(GroupType <>'TMR' and Department='LEISURE' ,AdultPax+ChildPax,0)) as PaxSeries,sum(if(GroupType <>'TMR' and Department<>'LEISURE' ,AdultPax+ChildPax,0)) as PaxPMT, sum(AdultPax+ChildPax) as Total,(sum(TotalPrice)+sum(DevPrice)) as Harga,sum((AdultPax+ChildPax)*TaxInsSell) as hargatax FROM tour_msbooking inner join tour_msproduct on (tour_msproduct.year =tour_msbooking.year and tour_msproduct.TourCode=tour_msbooking.TourCode) where tour_msproduct.Status <> 'VOID' and tour_msbooking.status='ACTIVE' and BookingStatus='DEPOSIT' and TCDivision<>'' and month(DateTravelFrom)=$mont and year(DateTravelFrom)=$yer and tour_msproduct.TourCode<>'' and Seat<>SeatSisa and TCDivision<>'LTM' group by TCDivision,Destination order by TCDivision");}else
	{
	$Booking=mysql_query("SELECT TCDivision,Destination,tour_msproduct.SellingCurr,sum(if(GroupType='TMR' ,AdultPax+ChildPax,0)) as PaxTMR,sum(if(GroupType <>'TMR' and Department='LEISURE' ,AdultPax+ChildPax,0)) as PaxSeries,sum(if(GroupType <>'TMR' and Department<>'LEISURE' ,AdultPax+ChildPax,0)) as PaxPMT, sum(AdultPax+ChildPax) as Total ,(sum(TotalPrice)+sum(DevPrice)) as Harga,sum((AdultPax+ChildPax)*TaxInsSell) as hargatax FROM tour_msbooking inner join tour_msproduct on (tour_msproduct.year =tour_msbooking.year and tour_msproduct.TourCode=tour_msbooking.TourCode) where tour_msproduct.Status <> 'VOID' and tour_msbooking.status='ACTIVE' and BookingStatus='DEPOSIT' and TCDivision<>'' and month(DateTravelFrom)=$mont and year(DateTravelFrom)=$yer and tour_msproduct.TourCode<>'' and Seat<>SeatSisa and TCDivision='$Div' and TCDivision<>'LTM' group by TCDivision,Destination order by Total desc");}}
	else{
	
	if ($Div=='ALL'){
	//Query tidak memunculkan tourcode yang kosong dan yang tidak terjual
	$Booking=mysql_query("SELECT TCDivision,Destination,tour_msproduct.SellingCurr,sum(if(GroupType='TMR' ,AdultPax+ChildPax,0)) as PaxTMR,sum(if(GroupType <>'TMR' and Department='LEISURE' ,AdultPax+ChildPax,0)) as PaxSeries,sum(if(GroupType <>'TMR' and Department<>'LEISURE' ,AdultPax+ChildPax,0)) as PaxPMT, sum(AdultPax+ChildPax) as Total,(sum(TotalPrice)+sum(DevPrice)) as Harga,sum((AdultPax+ChildPax)*TaxInsSell) as hargatax FROM tour_msbooking inner join tour_msproduct on (tour_msproduct.year =tour_msbooking.year and tour_msproduct.TourCode=tour_msbooking.TourCode) where tour_msproduct.Status <> 'VOID' and tour_msbooking.status='ACTIVE' and BookingStatus='DEPOSIT' and TCDivision<>'' and month(DateTravelFrom)=$mont and year(DateTravelFrom)=$yer and tour_msproduct.TourCode<>'' and Seat<>SeatSisa and TCDivision<>'LTM' and GroupType='$Gtype' group by TCDivision,Destination order by TCDivision");}else
	{
	$Booking=mysql_query("SELECT TCDivision,Destination,tour_msproduct.SellingCurr,sum(if(GroupType='TMR' ,AdultPax+ChildPax,0)) as PaxTMR,sum(if(GroupType <>'TMR' and Department='LEISURE' ,AdultPax+ChildPax,0)) as PaxSeries,sum(if(GroupType <>'TMR' and Department<>'LEISURE' ,AdultPax+ChildPax,0)) as PaxPMT, sum(AdultPax+ChildPax) as Total ,(sum(TotalPrice)+sum(DevPrice)) as Harga,sum((AdultPax+ChildPax)*TaxInsSell) as hargatax FROM tour_msbooking inner join tour_msproduct on (tour_msproduct.year =tour_msbooking.year and tour_msproduct.TourCode=tour_msbooking.TourCode) where tour_msproduct.Status <> 'VOID' and tour_msbooking.status='ACTIVE' and BookingStatus='DEPOSIT' and TCDivision<>'' and month(DateTravelFrom)=$mont and year(DateTravelFrom)=$yer and tour_msproduct.TourCode<>'' and Seat<>SeatSisa and TCDivision='$Div' and TCDivision<>'LTM' and GroupType='$Gtype' group by TCDivision,Destination order by Total desc");}
	}
     ...
     echo"<table style='border: 0px solid #000000;'> <tr><td colspan=6; style='border: 0px solid #000000;'><font size='2'> USD 1 = IDR $rate[$montText] ,-</td><td colspan=3; style='border: 0px solid #000000;'><strong>Group type :$Gtype </strong></font>
		...
		echo "<tr>
           ...
           <a href='?module=rptdivgroup&act=dtltourcode&BSO=$DBooking[TCDivision]&Dest=$DBooking[Destination]&Bln=$mont&thn=$yer&GType=$Gtype'>$TampilTotal</a>";}
		    if($DBooking[SellingCurr]=='USD'){$DHarga=($DBooking[Harga]+$DBooking[hargatax])*$rate[$montText];}else{$DHarga=($DBooking[Harga]+$DBooking[hargatax]);}
			 echo "</td>
	...
	case "dtltourcode":   
	if($_GET[GType]=='ALL'){
    $kriet=mysql_query("SELECT tour_msbooking.TourCode,Destination,ProductType,Department,sum(AdultPax+ChildPax) as Total ,(sum(TotalPrice)+sum(DevPrice)) as Harga,sum((AdultPax+ChildPax)*TaxInsSell) as hargatax FROM tour_msbooking inner join tour_msproduct on (tour_msproduct.year =tour_msbooking.year and tour_msproduct.TourCode=tour_msbooking.TourCode) where tour_msproduct.Status <> 'VOID' and tour_msbooking.status='ACTIVE' and BookingStatus='DEPOSIT' and month(DateTravelFrom)='$_GET[Bln]' and year(DateTravelFrom)='$_GET[thn]' and tour_msproduct.TourCode<>'' and Seat<>SeatSisa and TCDivision='$_GET[BSO]' and Destination='$_GET[Dest]' and TCDivision<>'LTM' group by tour_msbooking.TourCode order by Destination,Department,ProductType,Total desc");}
	else
	{
	 $kriet=mysql_query("SELECT tour_msbooking.TourCode,Destination,ProductType,Department,sum(AdultPax+ChildPax) as Total ,(sum(TotalPrice)+sum(DevPrice)) as Harga,sum((AdultPax+ChildPax)*TaxInsSell) as hargatax FROM tour_msbooking inner join tour_msproduct on (tour_msproduct.year =tour_msbooking.year and tour_msproduct.TourCode=tour_msbooking.TourCode) where tour_msproduct.Status <> 'VOID' and tour_msbooking.status='ACTIVE' and BookingStatus='DEPOSIT' and month(DateTravelFrom)='$_GET[Bln]' and year(DateTravelFrom)='$_GET[thn]' and tour_msproduct.TourCode<>'' and Seat<>SeatSisa and TCDivision='$_GET[BSO]' and Destination='$_GET[Dest]' and TCDivision<>'LTM' and GroupType='$_GET[GType]' group by tour_msbooking.TourCode order by Destination,Department,ProductType,Total desc");
	}
	...
	echo"<table>
			...
			while($sow=mysql_fetch_array($kriet)){   
                  	$TPax=$TPax+$sow[Total];
					$THarga=$THarga+(($sow[Harga]+$sow[hargatax])*$rate[$montText]);
				     echo"              
                     <tr>                                   
                     <td>$no</td>  
					 ...
					 <td>$sow[TourCode]</td> 
   					 ...
   					 <td style='text-align:right'>".number_format(($sow[Harga]+$sow[hargatax])*$rate[$montText], 0, ',', '.');echo"</td>
                     </tr>";
                      $no++;
                    };
					echo "
