...
switch($_GET[act]){
  ...
  default:
    ...
    echo "<h2>Settlement</h2>
          ...
          if(empty($halaman)){
            ...
            // Langkah 2
            $tampil=mysql_query("SELECT statuslog,KasNo,PRVCode,PRVNo,PRVCurr,PRVDate,SUM(PRVAmount)as Total FROM logsettle                                                  
                                WHERE (PRVNo LIKE '%$nama%'
                                OR PRVDate LIKE '%$nama%')  
                                Group BY PRVCode DESC  LIMIT $posisi,$batas");
            $jumlah=mysql_num_rows($tampil);
            
            if ($jumlah > 0) {
            echo "<table>
                    <tr><th>no</th><th>date</th><th>Settlement no.</th><th>Cash Adv No</th><th colspan=2>Amount</th><th>Status</th><th>action</th></tr>"; 
                  $no=$posisi+1;
               ...
               echo "<tr><td>$no</td>
                     ...
                     <center><a href=settle.php?id=$data[PRVCode]&usr=$username target=_blank>Show</a> | <a href=expset.php?id=$data[PRVCode]&usr=$username >Export</a> ";
                     if($data[statuslog]==0){echo" 
                     | <a href=\"javascript:voidfile('$data[PRVNo]','$data[PRVNo]')\">Void</a></tr>";
                    }
                    } 
                      $no++;
                    }
  ...
  case "editvisa":
    ...
    $edit=mysql_query("update msvisa set PRVNo = '', StatCancel = '0' WHERE PRVNo = '$_GET[id]'");
    $edit1=mysql_query("update logsettle set statuslog = '3' WHERE PRVNo = '$_GET[id]'");
