...
{
 window.location.href = '?module=mstargetpo&act=deletedetail&id=' + ID + '&no=' + no ;
...
switch($_GET[act]){
  ...
  default:
    ...
    echo "<h2>TARGET PO $targetyear</h2>
          <form method=get action='media.php?'>
                <input type=hidden name=module value='mstargetpo'>
              Year <select name='targetyear'>";                                  
            $tampil=mysql_query("SELECT * FROM tour_mstargetpo where TargetYear >= '$thnini' group by TargetYear ORDER BY TargetYear ASC");
    ...
    echo "</select>
          ...
          <input type=button value='New Period' onclick=location.href='?module=mstargetpo&act=tambahtarget' $visper> 
          <input type=button value='Insert New Destination' onclick=location.href='?module=mstargetpo&act=tambahpo'>";
          $oke=$_GET['oke'];      
          //pax
          $tampil=mysql_query("SELECT * FROM tour_mstargetpo   
                                left join tour_msproductcode on tour_msproductcode.ProductcodeArea = tour_mstargetpo.TargetBSO
                                WHERE TargetYear = '$targetyear'
                                AND ProductcodeStatus='ACTIVE'
                                group by ProductcodeArea
                                ORDER BY ProductcodeArea ASC,tour_mstargetpo.TargetBSO ASC");
            ...
            echo "<center><table class='bordered'><tr><th colspan='15'>TARGET PAX</th></tr>
               ...
               echo "<tr><td>$no</td>
                     <td><a href='?module=mstargetpo&act=edittarget&id=$data[TargetID]'>$data[TargetBSO]</a></td>                                   
                    ...
                    $tampils=mysql_query("SELECT sum(JAN)as TJAN,sum(FEB)as TFEB,sum(MAR)as TMAR,sum(APR)as TAPR,sum(MAY)as TMAY,sum(JUN)as TJUN,sum(JUL)as TJUL,
                                            sum(AUG)as TAUG,sum(SEP)as TSEP,sum(OCT)as TOCT,sum(NOV)as TNOV,sum(DES)as TDES FROM tour_mstargetpo   
                                WHERE TargetYear = '$targetyear'
                                ORDER BY tour_mstargetpo.TargetBSO ASC");
            ...
            //amount
            $tampils=mysql_query("SELECT * FROM tour_mstargetpo   
                                WHERE TargetYear = '$targetyear'
                                ORDER BY tour_mstargetpo.TargetBSO ASC");
            ...
            <center><table class='bordered'><tr><th colspan='15'>TARGET AMOUNT</th></tr>
               ...
               echo "<tr><td>$no</td>
                     <td><a href='?module=mstargetpo&act=edittarget&id=$datas[TargetID]'>$datas[TargetBSO]</a></td>                                  
                    ...
                    $tampils=mysql_query("SELECT sum(JANA)as TJAN,sum(FEBA)as TFEB,sum(MARA)as TMAR,sum(APRA)as TAPR,sum(MAYA)as TMAY,sum(JUNA)as TJUN,sum(JULA)as TJUL,
                                            sum(AUGA)as TAUG,sum(SEPA)as TSEP,sum(OCTA)as TOCT,sum(NOVA)as TNOV,sum(DESA)as TDES FROM tour_mstargetpo   
                                WHERE TargetYear = '$targetyear'
                                ORDER BY tour_mstargetpo.TargetBSO ASC");
                    $datas=mysql_fetch_array($tampils);
                    $totals=$datas[TJAN]+$datas[TFEB]+$datas[TMAR]+$datas[TAPR]+$datas[TMAY]+$datas[TJUN]+$datas[TJUL]+$datas[TAUG]+$datas[TSEP]+$datas[TOCT]+$datas[TNOV]+$datas[TDES]; 
  ...
  case "tambahtarget":
        $cuma = mysql_query("SELECT * FROM tour_mstargetpo group by TargetYear  
                                    ORDER BY TargetYear DESC limit 1");
        $saja = mysql_fetch_array($cuma);
        $neksyear=$saja[TargetYear]+1;    
    echo "<h2>New Target for $neksyear</h2>                                                                      
          
          <form name='example' method='POST' onsubmit='return validateFormOnSubmit(this)' action='./aksi.php?module=mstargetpo&act=input' >
          ...
          <tr><td colspan=2><center><input type='submit' name='submit' value=Create >
                            <input type=button value=Cancel onclick=location.href='?module=mstargetpo'></td></tr>
          </table> </form>
          <br><br>";
     break;
 ...
 case "tambahpo":
    ...
    echo "<h2>New Destination in target</h2>";
            echo "<form name='example' method='POST' onsubmit='return validateFormOnBSO(this)' action='./aksi.php?module=mstargetpo&act=insertpo' >
                    ...
                    $tampil=mysql_query("SELECT * FROM tour_msproductcode
                                         left outer join tour_mstargetpo on tour_mstargetpo.TargetBSO = tour_msproductcode.ProductcodeArea
                                         where ProductcodeStatus = 'ACTIVE'
                                         AND tour_mstargetpo.TargetID IS NULL
            ...
            echo "</select>
                    ...
                    <center><input type='submit' name='submit' value='Insert'> <input type=button value='Cancel' onclick=location.href='?module=mstargetpo&targetyear=$data[TargetYear]'>
                    </form><br><br>";  
     break;
 ...
 case "edittarget":
    ...
    echo "<h2>TARGET $data[TargetBSO] $data[TargetYear]</h2>";          
            echo "<form name='example' method='POST' onsubmit='return validateFormOnBlank(this)' action='./aksi.php?module=mstargetpo&act=update' >  
                    ...
                    <input type='submit' name='submit' value='Update'> <input type=button value='Cancel' onclick=location.href='?module=mstargetpo&targetyear=$data[TargetYear]'>
                    </form><br><br>";         
     break;
