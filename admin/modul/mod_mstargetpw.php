<script language="JavaScript"  type="text/javascript">
    ...
    function validateFormOnBSO(theForm) {
        var reason = "";
        reason += validateSelect(theForm.targetpw);
        reason += validateEmpty(theForm.jan);
        reason += validateEmpty(theForm.feb);
        reason += validateEmpty(theForm.mar);
        reason += validateEmpty(theForm.apr);
...
switch($_GET[act]) {
    ...
    default:
        ...
        echo "<h2>TARGET PW $targetyear</h2>
          <form method=get action='media.php?'>
                <input type=hidden name=module value='mstargetpw'>
              Year <select name='targetyear'>";
        $tampil = mysql_query("SELECT * FROM tour_mstargetpw where TargetYear >= '$thnini' group by TargetYear ORDER BY TargetYear ASC");
        ...
        echo "</select>
          ...
          <input type=button value='New Period' onclick=location.href='?module=mstargetpw&act=tambahtarget'>";
        $oke = $_GET['oke'];
        // pax
        $qdivpanel = mssql_query("SELECT DivisiID FROM [HRM].[dbo].[Divisi]
                                  WHERE Category = 'SALES OUTLET' AND CompanyGroup ='PANORAMA WORLD'
                                  AND Active = 1
        ...
        if ($divtampil > 0) {
            echo "<center><table style='border: 0px solid #000000;'><td style='border: 0px solid #000000;'>
                    ...
                    <tr><th>no</th><th>pw</th><th>Jan</th><th>Feb</th><th>Mar</th><th>Apr</th><th>May</th><th>Jun</th><th>Jul</th><th>Aug</th><th>Sep</th><th>Oct</th><th>Nov</th><th>Des</th><th>total</th></tr>";
            $no = $posisi + 1;
            $jumlah=0;
            while ($divpanel = mssql_fetch_array($qdivpanel)) {
                $tampil = mysql_query("SELECT * FROM tour_mstargetpw
                                WHERE TargetYear = '$targetyear'
                                AND TargetBSO = '$divpanel[DivisiID]'
                                ORDER BY TargetBSO ASC");
                $jumlah = mysql_num_rows($tampil);
                $data = mysql_fetch_array($tampil);
                if($jumlah>0) {$klik="?module=mstargetpw&act=edittarget&id=$data[TargetID]";}
                else {$klik="?module=mstargetpw&act=tambahpw&pw=$divpanel[DivisiID]";}
                $totpw = $data[JAN] + $data[FEB] + $data[MAR] + $data[APR] + $data[MAY] + $data[JUN] + $data[JUL] + $data[AUG] + $data[SEP] + $data[OCT] + $data[NOV] + $data[DES];
                echo "<tr><td>$no</td>
                     ...
                     <td><center><font color=red>$totpw</font></td>
                     </tr>";
                $no++;
                // }
            }
            ...
            while($divactive = mssql_fetch_array($qdiv)) {
                $tampils = mysql_query("SELECT JAN,FEB,MAR,APR,MAY,JUN,JUL,AUG,SEP,OCT,NOV,DES FROM tour_mstargetpw
                ...
                $tampilj = mysql_query("SELECT JAN,FEB,MAR,APR,MAY,JUN,JUL,AUG,SEP,OCT,NOV,DES,JANA,FEBA,MARA,APRA,MAYA,JUNA,JULA,AUGA,SEPA,OCTA,NOVA,DESA FROM tour_mstargetpw
                                WHERE TargetYear = '$targetyear'
                ...
                $tampiln = mysql_query("SELECT JAN,FEB,MAR,APR,MAY,JUN,JUL,AUG,SEP,OCT,NOV,DES,JANA,FEBA,MARA,APRA,MAYA,JUNA,JULA,AUGA,SEPA,OCTA,NOVA,DESA FROM tour_mstargetpw
                                WHERE TargetYear = '$targetyear'
            ...
            echo "
                    ...
                    <tr><th>month</th><th><a href='?module=mstargetpw&act=editrate&id=$ret[RateID]' style='color:white'>rate USD</a></th><th>pax</th><th>amount</th><th>pax</th><th>amount</th></tr>
        ...
        //amount
        $tampils = mysql_query("SELECT * FROM tour_mstargetpw
                                WHERE TargetYear = '$targetyear'
                                ORDER BY tour_mstargetpw.TargetBSO ASC");
        ...
        if ($jumlahs > 0) {
            ...
            while ($datas = mysql_fetch_array($tampils)) {
                $qdivpanel = mssql_query("SELECT DivisiID FROM [HRM].[dbo].[Divisi] WHERE DivisiID = '$datas[TargetBSO]' and Active = 1 ");
                $divtampil = mssql_num_rows($qdivpanel);
                if($divtampil>0) {
                    $divpanel = mssql_fetch_array($qdivpanel);
                    $totpw = $datas[JANA] + $datas[FEBA] + $datas[MARA] + $datas[APRA] + $datas[MAYA] + $datas[JUNA] + $datas[JULA] + $datas[AUGA] + $datas[SEPA] + $datas[OCTA] + $datas[NOVA] + $datas[DESA];
                    echo "<tr><td>$no</td>
                     <td><a href='?module=mstargetpw&act=edittarget&id=$datas[TargetID]'>$divpanel[DivisiID]</a></td>
                    ...
                    echo "</td>
                     <td style='text-align:right'><font color=red>" . number_format($totpw, 0, '', '.');
                    echo "</font></td></tr>";
                    $no++;
                }
            }
            $qdiv = mssql_query("SELECT DivisiID FROM [HRM].[dbo].[Divisi] WHERE Active = 1 ");
            while($divactive = mssql_fetch_array($qdiv)) {
                $tampils = mysql_query("SELECT JANA,FEBA,MARA,APRA,MAYA,JUNA,JULA,AUGA,SEPA,OCTA,NOVA,DESA FROM tour_mstargetpw
                                WHERE TargetYear = '$targetyear' AND TargetBSO = '$divactive[DivisiID]' ");
                $datas = mysql_fetch_array($tampils);
    ...
    case "tambahtarget":
        $cuma = mysql_query("SELECT * FROM tour_mstargetpw group by TargetYear  
                                    ORDER BY TargetYear DESC limit 1");
        $saja = mysql_fetch_array($cuma);
        $neksyear = $saja[TargetYear] + 1;
        echo "<h2>New Target for $neksyear</h2>
          
          <form name='example' method='POST' onsubmit='return validateFormOnSubmit(this)' action='./aksi.php?module=mstargetpw&act=input' >
          ...
          <tr><td colspan=2><center><input type='submit' name='submit' value=Create >
                            <input type=button value=Cancel onclick=location.href='?module=mstargetpw'></td></tr>
          </table> </form>
          <br><br>";
        break;
    ...
    case "tambahpw":
        ...
        $thnini = date("Y");
        $pw=$_GET['pw'];
        echo "<h2>New PW in target</h2>";
        echo "<form name='example' method='POST' onsubmit='return validateFormOnBSO(this)' action='./aksi.php?module=mstargetpw&act=insertpw' >
                    <input type='hidden' name='thn' value='$thnini'>
                    <center>New PW <input type='text' name='targetpw' value='$pw'>
                    ...
                    <center><input type='submit' name='submit' value='Insert'> <input type=button value='Cancel' onclick=location.href='?module=mstargetpw&targetyear=$data[TargetYear]'>
                    </form><br><br>";
        break;
    ...
    case "edittarget":
        ...
        echo "<form name='example' method='POST' onsubmit='return validateFormOnBlank(this)' action='./aksi.php?module=mstargetpw&act=update' >
                    ...
                    <input type='submit' name='submit' value='Update'> <input type=button value='Cancel' onclick=location.href='?module=mstargetpw&targetyear=$data[TargetYear]'>
                    </form><br><br>";
        break;
    ...
    case "editrate":
        ...
        echo "<h2>Edit Rate for $data[RateYear]</h2>
          ...
          <center><table><tr><th>month</th><th width='50'>USD</th><th>IDR</th></tr>
                    ...
                    <center><input type='submit' name='submit' value='Update'> <input type=button value='Cancel' onclick=location.href='?module=mstargetpw&targetyear=$data[TargetYear]'>
                    </form><br><br>";
        break;
}
?>
