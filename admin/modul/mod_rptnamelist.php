		...
		if($IDProduct<>''){
         ...
         $totroom=mysql_num_rows($totrum);
            ...
            echo "<table STYLE='border: 0px solid #000000' id='namelist'>	
                    ...
                    <th colspan=2>place & Issuing Date Passport</th></tr>";
                   ...
                   if($isi[TourLeaderInc]=='YES'){
                       ...
                       while($tlnya=mysql_fetch_array($cariteel)){
                            ...
                            echo "<tr><td>$notl</td>
                                <td></td> 
                                <td>$datatl[Title] $datatl[NameAsPassport]</td>
                                <td><center>SGL</td>
                                <td></td>
                                <td></td>
                    ...
                    while ($data=mysql_fetch_array($edit)){
                        ...
                        if($data[Promo]<>'' AND $vchok > 0){$promo="<font color='red'>*</font>";$ket="<font size=1>* $data[Promo]</font>";}else{$promo="";}
		                if($data[Age]<>'' and ($data[Gender]=='CHILD' OR $data[Gender]=='INFANT')){$age="<b>($data[Age])</b>";}else{$age="";}
               ...
               echo "<tr><td>$no</td>";
                ...
                echo"<td>$inf $data[Title] $pax $age</td>";
				  ...
				  if($roomnow<>$data[RoomNo] or $lastbooking<>$data[BookingID] ){
					...
					//gabungan jenis kamar
                        ...
                        <td rowspan='$jumlahparty' style=vertical-align:middle><center>$data[RoomNo]</td>";
						$roomnow=$data[RoomNo];}
                   ...
                   // if($isi[StatusProduct]=='FINALIZE'){$ganti='enable';}else{$ganti='disabled';}
                    ...
                    <input type=button value='Change Room' onclick=location.href='?module=rptnamelist&act=roomchange&code=$nama'>";
	}
	}        
	break;
...
case "namelistgrv":
    ...
    case "roomchange":
        ...
        $edit=mysql_query("SELECT tour_msbookingdetail.*,tour_msbooking.TotalRoom,CONVERT(RoomNo, UNSIGNED INTEGER) as urut FROM tour_msbookingdetail left join tour_msbooking on tour_msbooking.BookingID = tour_msbookingdetail.BookingID
                    ...
                    AND tour_msbooking.Status = 'ACTIVE'
                    AND tour_msbookingdetail.Status <> 'CANCEL'
                    order by tour_msbooking.BookingID ASC,urut,IDDetail ASC");
        ...
        if ($jumlah > 0) {
            ...
            while ($data=mysql_fetch_array($edit)){
				...
				<td style=vertical-align:middle><center>$data[BookingID]<br>($data[TotalRoom] Room)</center></td>
